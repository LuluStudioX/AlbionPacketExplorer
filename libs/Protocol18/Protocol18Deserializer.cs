using AlbionPacketExplorer.Protocol18.Photon;
using System.Collections;
using System.Text;

namespace AlbionPacketExplorer.Protocol18;
...
public static class Protocol18Deserializer
{
    ...
    private static bool DeserializeBoolean(Stream input) => ReadByte(input) != 0;

    private static float DeserializeFloat(Stream input)
    {
        byte[] buffer = GetScalarBuffer();
        ReadExactly(input, buffer, sizeof(float));
        if (!BitConverter.IsLittleEndian) Array.Reverse(buffer, 0, sizeof(float));
    ...
    private static double DeserializeDouble(Stream input)
    {
        ...
        if (!BitConverter.IsLittleEndian) Array.Reverse(buffer, 0, sizeof(double));
    ...
    private static short[] DeserializeShortArray(Protocol18Stream input)
    {
        ...
        for (int i = 0; i < arrayLength; i++) array[i] = DeserializeShort(input);
        return array;
    }
    ...
    private static string[] DeserializeStringArray(Protocol18Stream input)
    {
        ...
        for (int i = 0; i < arrayLength; i++) array[i] = DeserializeString(input);
        return array;
    }
    ...
    private static int[] DeserializeCompressedIntArray(Protocol18Stream input)
    {
        ...
        for (int i = 0; i < arrayLength; i++) array[i] = ReadCompressedInt32(input);
        return array;
    }
    ...
    private static long[] DeserializeCompressedLongArray(Protocol18Stream input)
    {
        ...
        for (int i = 0; i < arrayLength; i++) array[i] = ReadCompressedInt64(input);
        return array;
    }
    ...
    private static object[] DeserializeObjectArray(Protocol18Stream input)
    {
        ...
        for (int i = 0; i < arrayLength; i++) array[i] = Deserialize(input)!;
        return array;
    }
    ...
    private static Hashtable[] DeserializeHashtableArray(Protocol18Stream input)
    {
        ...
        for (int i = 0; i < arrayLength; i++) array[i] = DeserializeHashtable(input);
        return array;
    }
    ...
    private static Array? DeserializeArrayInArray(Protocol18Stream input)
    {
        ...
        for (int i = 0; i < arrayLength; i++)
        {
            ...
            if (value is not Array nestedArray) continue;
            if (result == null)
            {
    ...
    private static void DeserializeDictionaryElements(Protocol18Stream input, IDictionary dictionary, Protocol18Type keyTypeCode, Protocol18Type valueTypeCode)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            object? key = keyTypeCode == Protocol18Type.Unknown ? Deserialize(input) : Deserialize(input, (byte) keyTypeCode);
            object? value = valueTypeCode == Protocol18Type.Unknown ? Deserialize(input) : Deserialize(input, (byte) valueTypeCode);
            if (key != null) dictionary.Add(key, value);
        }
    }
    ...
    private static Type DeserializeDictionaryType(Protocol18Stream input, out Protocol18Type keyTypeCode, out Protocol18Type valueTypeCode)
    {
        ...
        Type keyType = keyTypeCode == Protocol18Type.Unknown ? typeof(object) : GetAllowedDictionaryKeyType(keyTypeCode);
        ...
        if (valueTypeCode == Protocol18Type.Array) valueTypeCode = Protocol18Type.Unknown;
    ...
    private static Type GetDictionaryArrayType(Protocol18Stream input)
    {
        ...
        for (int i = 0; i < nestedArrayDepth; i++) arrayType = arrayType.MakeArrayType();
        return arrayType;
    }

    private static Type GetAllowedDictionaryKeyType(Protocol18Type typeCode) => typeCode switch
    {
    ...
    private static Type GetClrArrayType(Protocol18Type typeCode) => typeCode switch
    {
    ...
    {
        byte typeCode = slimTypeCode == 0 ? ReadByte(input) : (byte) (slimTypeCode - (byte) Protocol18Type.CustomTypeSlim);
    ...
    private static int ReadInt1(Stream input, bool signNegative) { int v = ReadByte(input); return signNegative ? -v : v; }
    private static int ReadInt2(Protocol18Stream input, bool signNegative) { int v = ReadUShort(input); return signNegative ? -v : v; }
    private static long ReadLong1(Stream input, bool signNegative) { long v = ReadByte(input); return signNegative ? -v : v; }
    private static long ReadLong2(Protocol18Stream input, bool signNegative) { long v = ReadUShort(input); return signNegative ? -v : v; }
    private static int ReadCompressedInt32(Protocol18Stream input) => DecodeZigZag32(ReadCompressedUInt32(input));
    private static long ReadCompressedInt64(Protocol18Stream input) => DecodeZigZag64(ReadCompressedUInt64(input));

    private static uint ReadCompressedUInt32(Protocol18Stream input)
    {
        uint value = 0; int shift = 0;
        while (shift != 35) { byte current = ReadByte(input); value |= (uint) (current & 0x7F) << shift; shift += 7; if ((current & 0x80) == 0) return value; }
        return value;
    }
    ...
    private static ulong ReadCompressedUInt64(Protocol18Stream input)
    {
        ...
        while (shift != 70) { byte current = ReadByte(input); value |= (ulong) (current & 0x7F) << shift; shift += 7; if ((current & 0x80) == 0) return value; }
        return value;
    }
    ...
    private static byte ReadByte(Stream input)
    {
        ...
        if (value < 0) throw new EndOfStreamException("Failed to read a byte from the Protocol18 payload.");
        return (byte) value;
    }
    ...
    private static void ReadExactly(Stream input, byte[] buffer, int count)
    {
        ...
        while (offset < count)
        {
            ...
            if (read <= 0) throw new EndOfStreamException($"Failed to read {count} bytes from the Protocol18 payload.");
            offset += read;
        }
    }

    private static byte[] GetScalarBuffer() => ScalarBuffer.Value ?? throw new InvalidOperationException("The scalar Protocol18 buffer could not be created.");
    private static int DecodeZigZag32(uint value) => (int) ((value >> 1) ^ (0u - (value & 1u)));
    private static long DecodeZigZag64(ulong value) => (long) ((value >> 1) ^ (0UL - (value & 1UL)));
}
...
internal sealed class Protocol18CustomType(byte typeCode, byte[] data)
{
    ...
    public override string ToString() => $"Protocol18CustomType({TypeCode}, {Data.Length} bytes)";
}
