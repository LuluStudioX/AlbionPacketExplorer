...
4. No way to export discovered knowledge back to SAT/community
...
|------|----------|---------|
| `packet-schema.base.json` | `src/.../Assets/` (embedded resource) | Auto-generated from SAT handlers. Read-only. Shipped with app. |
...
1. Reads SAT `Network/Events/*.cs` and `Network/Handler/*.cs`
2. Extracts param key → property name mappings via Roslyn or regex
3. Outputs `packet-schema.base.json`
4. Run manually when SAT repo updates — output committed to repo
...
Standalone file, SAT-compatible annotation:
...
This format is directly usable for a SAT PR — the handler author can read it and implement the DTO.
...
|------|-------|-------------|
| 1 | Generate `packet-schema.base.json` from extracted SAT data | `packet(assets)` |
...
- **Schema export**: full merged schema always — most useful for SAT PRs.
